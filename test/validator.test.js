...
describe('测试 @Validator', () => {
  ...
  test('测试 Credential.prototype.validate(null)', () => {
    const credential = new Credential(CredentialType.PASSPORT.value, 'E12345678');
    console.log('credential = ', credential);
    let result = credential.validate(null);
    expect(result).toBeInstanceOf(ValidationResult);
    expect(result.success).toBe(true);
    expect(result.description).toBe('');

    credential.assign({ type: 'xxx' });
    result = credential.validate(null);
    expect(result).toBeInstanceOf(ValidationResult);
    expect(result.success).toBe(false);
    expect(result.description).toBe('证件类型不受支持："xxx"');
  ...
  test('测试 Credential.prototype.validate("number")', () => {
    const credential = new Credential(CredentialType.PASSPORT.value, 'E12345678');
    console.log('credential = ', credential);
    let result = credential.validate('number');
    expect(result).toBeInstanceOf(ValidationResult);
    expect(result.success).toBe(true);
    expect(result.description).toBe('');

    credential.assign({ type: 'xxx', number: '123' });
    result = credential.validate('number');
    expect(result).toBeInstanceOf(ValidationResult);
    expect(result.success).toBe(true);
    expect(result.description).toBe('');
